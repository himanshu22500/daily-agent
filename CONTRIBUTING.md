...
4. Open a PR: `Closes #N`, what you did, and **what's verified offline vs needs
   live verification**. Label `needs-local-verification` if it touches Huly /
   Telegram / live data / real LLM output.
...
cp .env.example .env         # then fill in for live use (GitHub/Huly/Telegram/LLM)
```
- The Huly bridge needs Node: `cd bridges/huly && yarn install` (once).
