...
def _issue(identifier, title, parents=None, prs=None):
    return {
...
def test_catalog_is_distinct_initiatives_only():
    issues = [
        _issue("pm#10", "Sub A", [("pm#1", "Billing Revamp")]),
        _issue("pm#11", "Sub B", [("pm#1", "Billing Revamp")]),  # same initiative
        _issue("pm#2", "Standalone Feature"),  # itself
        _issue("pm#12", "fix", [("pm#3", "Project Oncall [Jun]")]),  # ops, excluded
    ]
    catalog = build_catalog(issues)
    keys = {c.key for c in catalog}
    assert keys == {"pm#1", "pm#2"}  # deduped; ops excluded
    assert all(c.lane == "initiative" for c in catalog)


# --- orchestrator ---------------------------------------------------------- #
@pytest.mark.asyncio
async def test_linked_pr_anchored_without_llm(monkeypatch):
    called = {"n": 0}
    ...
    # The PR's title carries no ticket — the link is native (closedByPullRequests).
    issues = [_issue("pm#1", "Billing Revamp", prs=[("api", 1)])]
    prs = [_pr("api", 1, "feat(billing): add invoices")]
    out = await resolve_initiatives("m", prs, issues)
    assert out["api#1"].key == "pm#1"
...
async def test_orphan_pr_mapped_by_llm_onto_catalog(monkeypatch):
    async def fake_map(model, prs, catalog):
        return {"api#2": "pm#1"}  # LLM places the scoped PR onto Billing Revamp

    monkeypatch.setattr(mapping_mod, "map_orphans", fake_map)
    issues = [_issue("pm#1", "Billing Revamp")]  # no linked PRs -> api#2 is an orphan
    prs = [_pr("api", 2, "feat(billing): add proration")]
    out = await resolve_initiatives("m", prs, issues)
    assert out["api#2"].key == "pm#1"
...
async def test_llm_untracked_and_invalid_key_fall_to_untracked(monkeypatch):
    async def fake_map(model, prs, catalog):
        return {"api#3": "untracked", "api#4": "pm#DOES-NOT-EXIST"}

    monkeypatch.setattr(mapping_mod, "map_orphans", fake_map)
    issues = [_issue("pm#1", "Billing Revamp")]
...
async def test_cached_key_absent_from_catalog_falls_to_untracked(monkeypatch, tmp_path):
    ...
    async def fake_map(model, prs, catalog):
        return {f"{p.repo}#{p.number}": "pm#99" for p in prs}  # not in catalog later

    monkeypatch.setattr(mapping_mod, "map_orphans", fake_map)
    cache = Cache(tmp_path / "c.db")
    prs = [_pr("api", 7, "feat: mystery")]

    # First run: a catalog where pm#99 is a valid key, so it gets cached.
    await resolve_initiatives("m", prs, [_issue("pm#99", "Old Thing")], cache=cache)
    # Second run: pm#99 no longer in the catalog -> untracked, no crash, no LLM.
    out = await resolve_initiatives(
        "m", prs, [_issue("pm#1", "New Thing")], cache=cache
    )
...
async def test_oncall_pr_routed_to_ops(monkeypatch):
    ...
    monkeypatch.setattr(mapping_mod, "map_orphans", fake_map)
    issues = [
        _issue("pm#5", "hotfix", [("pm#6", "Project Oncall [Jun]")], prs=[("api", 5)])
    ]
    prs = [_pr("api", 5, "hotfix")]
    out = await resolve_initiatives("m", prs, issues)
