...
A hybrid: PRs that natively close a tracked GitHub Project issue (via
``closedByPullRequestsReferences``) are anchored deterministically by inverting
that link; the remaining ticket-less PRs are assigned by the LLM onto the
GitHub-Projects-derived catalog. Anything the LLM can't place stays untracked —
...
async def resolve_initiatives(
    ...
    *,
    ...
    Deterministic first: PRs that close a tracked issue (or land in ops) are
    anchored without the LLM. The rest are mapped onto the catalog by the LLM;
    unmatched ones fall to the untracked lane.
    ...
    """
    pr_to_issue = index_by_linked_pr(issues)
    ...
    for pr in prs:
        init = initiative_for_pr(pr, pr_to_issue)
